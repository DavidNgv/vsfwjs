...
(function() {
  ...
  fs.readFileSync = function(p, encoding) {
    var _ref;
    return (_ref = vsfw_fs[p]) != null ? _ref : native_readFileSync.apply(fs, native_array.apply(null, arguments));
  };
  ...
  path.existsSync = fs.existsSync = function(p) {
    return (vsfw_fs[p] != null) || native_existsSync.apply(fs, native_array.apply(null, arguments));
  };
  ...
  vsfw.app = function(func, options) {
    var app, apply_helpers, client, client_bundle_simple, client_bundled, context, helpers, io, partials, postrenders, real_root, request_socket, root, route, socket_session, verb, vsfw_prefix, vsfw_used, ws_handlers, _fn, _i, _len, _ref2;
    if (options == null) {
      options = {};
    }
    context = {
    ...
    context["with"] = function(obj) {
      var k, v, vsfw_with, _results;
      vsfw_with = {
        css: function(modules) {
          var module, name, _j, _len1;
    ...
    context.use = function() {
      var a, k, use, v, vsfw_middleware, _j, _len1;
      vsfw_middleware = {
        "static": function(options) {
          ...
          p = (_ref3 = options.path) != null ? _ref3 : path.join(real_root, '/public');
          delete options.path;
        ...
        vsfw: function() {
          ...
          return function(req, res, next) {
            ...
            if (req.method.toUpperCase() !== 'GET') {
              ...
              switch (req.url) {
                case vsfw_prefix + '/Vsfw.js':
                  send(client_bundled);
                  break;
                case vsfw_prefix + '/Vsfw-simple.js':
                  send(client_bundle_simple);
                  break;
                case vsfw_prefix + '/vsfw.js':
                  send(client);
                  break;
                case vsfw_prefix + '/jquery.js':
                  send(jquery_minified);
                  break;
                case vsfw_prefix + '/sammy.js':
                  send(sammy_minified);
                  break;
                default:
                  next();
      ...
      use = function(name, arg) {
        ...
        if (vsfw_middleware[name]) {
          return app.use(vsfw_middleware[name](arg));
    ...
    request_socket = function(req) {
      var socket_id, _ref3, _ref4, _ref5;
      socket_id = (_ref3 = req.session) != null ? (_ref4 = _ref3.__socket) != null ? (_ref5 = _ref4['__local']) != null ? _ref5.id : void 0 : void 0 : void 0;
      ...
      context.view({
        layout: function() {
          ...
          return html(function() {
            head(function() {
              var s, _j, _k, _len1, _len2, _ref3, _ref4;
              if (this.title) {
                title(this.title);
              }
              if (this.scripts) {
    ...
    if (io != null) {
      vsfw_prefix = app.settings['vsfw_prefix'];
      context.get(vsfw_prefix + '/socket/:channel_name/:socket_id', function() {
  ...
  vsfw.run = function() {
    var a, app, express_ready, host, k, options, port, root_function, v, vsapp, _i, _len;
    host = null;
    port = 3000;
    root_function = null;
    options = {
    ...
    express_ready = function() {
      var _ref2;
      log('Express server listening on port %d in %s mode', (_ref2 = vsapp.server.address()) != null ? _ref2.port : void 0, app.settings.env);
      return log("vSoft Framework " + vsfw.version + " \"" + codename + "\" orchestrating the show");
    };
    if (host) {
  ...
  vsfw.adapter = function(engine, options) {
